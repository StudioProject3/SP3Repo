...
{
public:
	static Mesh* GenerateAxes(const std::string &meshName, float lengthX=0.0f, float lengthY=0.0f, float lengthZ=0.0f, float alpha = 1.0f);
	static Mesh* GenerateCrossHair(const std::string &meshName, float colour_r=1.0f, float colour_g=1.0f, float colour_b=0.0f, float length=1.0f, float alpha = 1.0f);
	...
	static Mesh* GenerateTerrain(const std::string &meshNameconst, const string &filepath, vector<unsigned char> &heightmap, float alpha = 1.0f);
	static Mesh* MeshBuilder::GenerateHemisphere(const std::string &meshName, Color color, unsigned numStack, unsigned numSlice, float radius);
	static SpriteAnimation* MeshBuilder::GenerateSpriteAnimation(const std::string &meshName, unsigned numRow, unsigned numCol, float alpha = 1.0f);
};
