...
void SceneMainScene::Init()
{
	...
	lights[1].position.Set(-160.f, -48.f, -120.f);
	lights[1].color.Set(0.84f, 0.28f, 0, 1);
	lights[1].power = 800;
	lights[1].kC = 10.f;
	lights[1].kL = 0.01f;
	lights[1].kQ = 0.001f;
...
float SceneMainScene::RotateX(GameObject* Object)
{
	Vector3 frontZ;
	Vector3 backZ;
	Vector3 centerZ;
...
float SceneMainScene::RotateZ(GameObject* Object)
{
	Vector3 frontX;
	Vector3 backX;
	Vector3 centerX;
...
GameObject* SceneMainScene::FetchGO()
{
...
void SceneMainScene::Update(double dt)
{
	...
	if(Application::IsKeyPressed('I'))
		lights[1].position.z -= (float)(100.f * dt);
	if(Application::IsKeyPressed('K'))
		lights[1].position.z += (float)(100.f * dt);
	if(Application::IsKeyPressed('J'))
		lights[1].position.x -= (float)(100.f * dt);
	if(Application::IsKeyPressed('L'))
		lights[1].position.x += (float)(100.f * dt);
	if(Application::IsKeyPressed('O'))
		lights[1].position.y -= (float)(100.f * dt);
...
void SceneMainScene::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SceneMainScene::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
...
void SceneMainScene::RenderMeshIn2D(Mesh *mesh, bool enableLight, float size, float x, float y)
{
	Mtx44 ortho;
...
void SceneMainScene::RenderMesh(Mesh *mesh, bool enableLight)
{
	Mtx44 MVP, modelView, modelView_inverse_transpose;
...
void SceneMainScene::RenderTerrain()
{
	modelStack.PushMatrix();
...
void SceneMainScene::RenderWorld()
{
	modelStack.PushMatrix();
...
void SceneMainScene::RenderPassGPass()
{
	...
	//These matrices should change when light position or direction changes
		if (lights[0].type == Light::LIGHT_DIRECTIONAL)
			m_lightDepthProj.SetToOrtho(-100, 100, -100, 100, -100, 400);
		else
			m_lightDepthProj.SetToPerspective(90, 1.f, 0.1, 200);
			m_lightDepthView.SetToLookAt(lights[0].position.x, lights[0].position.y, lights[0].position.z, 0, 0, 0, 0, 1, 0);

		RenderWorld(); //Objects with shadow
}

void SceneMainScene::RenderPassMain()
{
