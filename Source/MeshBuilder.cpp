...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateAxes(const std::string &meshName, float lengthX, float lengthY, float lengthZ, float alpha)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	v.pos.Set(-1000, 0, 0);
	v.color.Set(1, 0, 0, alpha);
	vertex_buffer_data.push_back(v);
	v.pos.Set(1000, 0, 0);
	v.color.Set(1, 0, 0, alpha);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0, -1000, 0);
	v.color.Set(0, 1, 0, alpha);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0, 1000, 0);
	v.color.Set(0, 1, 0, alpha);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0, 0, -1000);
	v.color.Set(0, 0, 1, alpha);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0, 0, 1000);
	v.color.Set(0, 0, 1, alpha);
	vertex_buffer_data.push_back(v);
...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateCrossHair(const std::string &meshName, float colour_r, float colour_g, float colour_b, float length, float alpha)
{
	...
	v.color.Set(colour_r, colour_g, colour_b, alpha);
	vertex_buffer_data.push_back(v);
	// Vertex #2
	v.pos.Set(length, 0, 0);
	v.color.Set(colour_r, colour_g, colour_b, alpha);
...
Mesh* MeshBuilder::GenerateSkyPlane(const std::string &meshName, Color color, int slices, float PlanetRadius, float AtmosphereRadius, float hTile, float vTile)
{
	...
	for (int i = 0; i < slices; ++i)
	{
		...
		{
			int starVert = (i * (slices + 1) + j);
			//Triangle 1
			index_buffer_data.push_back(starVert);
			index_buffer_data.push_back(starVert + 1);
			index_buffer_data.push_back(starVert + slices + 1);

			//Triangle 2
			index_buffer_data.push_back(starVert + 1);
			index_buffer_data.push_back(starVert + slices + 2);
			index_buffer_data.push_back(starVert + slices + 1);
		}
	}
...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, const string &filepath, vector<unsigned char> &heightmap, float alpha)
{
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	for (unsigned z = 0;z < terrainSize;++z)
	{
		...
		{
			float scaledHeight = (unsigned char)heightmap[z * terrainSize + x]/SCALE_FACTOR;

			v.pos.Set(static_cast<float>(x) / terrainSize - 0.5f, scaledHeight, static_cast<float>(z) / terrainSize - 0.5f);
			v.color.Set(0.2f, scaledHeight * 2, 0.8f, alpha);
			v.texCoord.Set((float)x / terrainSize * 30, 1.f - (float)z / terrainSize * 30);
